...
def quantize_1_bit(arr: np.ndarray) -> np.ndarray:
    quantized_arr = [0] * len(arr)
    for i in range(len(arr)):
        real_part = arr[i].real
        if arr[i].real>=0:
            real_part = 1
        else:
            real_part = -1
        if arr[i].imag>=0:
            ...
            imag_part = -1
        quantized_arr[i] = complex(real_part, imag_part)
    return quantized_arr

def quantize_2_bit(arr: np.ndarray) -> np.ndarray:
    quantized_arr = [0] * len(arr)
    for i in range(len(arr)):
        real_part = arr[i].real
        if arr[i].real>=5:
...
snr_values = [-20, -10, -5, -3, 1, 3, 5, 10, 20]
for snr in snr_values:
    ...
    average_code_phase_estimate_navic_1bit = [0.0] * 10
    average_code_phase_estimate_gps_1bit = [0.0] * 10
    average_doppler_estimate_navic_1bit = [0.0] * 10
    average_doppler_estimate_gps_1bit = [0.0] * 10

    average_code_phase_estimate_navic_2bit = [0.0] * 10
    average_code_phase_estimate_gps_2bit = [0.0] * 10
    average_doppler_estimate_navic_2bit = [0.0] * 10
    average_doppler_estimate_gps_2bit = [0.0] * 10
    num_iterations = 10
    ...
    for a in range(num_iterations):
        ...
        navic_sequence_1bit = quantize_1_bit(navic_sequence)
        navic_sequence_2bit = quantize_2_bit(navic_sequence)
        ...
        gps_sequence_1bit = quantize_1_bit(gps_sequence)
        gps_sequence_2bit = quantize_2_bit(gps_sequence)
        ...
        navic_doppler_estimates_1bit = []
        gps_doppler_estimates_1bit = []
        navic_code_phase_estimates_1bit = []
        gps_code_phase_estimates_1bit = []
        for i in range(10):
            print(f"Satellite: {i + 1}")
            print(f"Actual Doppler: {doppler[i]}")
            print(f"Code Phase Delay: {timedelay[i]}")
            ...
            print("Navic Estimate 1 bit:")
            navic_estimates_1bit = code_phase_doppler_estimate(navic_sequence_1bit, 4e6, doppler_estimate_10, prn10_cpe_navic_complex,threshold)
            code_phase_estimate_navic_1bit = navic_estimates_1bit[0]
            doppler_estimate_navic_1bit = navic_estimates_1bit[1]

            print("GPS Estimate 1 bit:")
            gps_estimates_1bit = code_phase_doppler_estimate(gps_sequence_1bit, 4e6, doppler_estimate_10, prn10_cpe_gps_complex,threshold)
            code_phase_estimate_gps_1bit = gps_estimates_1bit[0]
            doppler_estimate_gps_1bit = gps_estimates_1bit[1]

            navic_code_phase_estimates_1bit.append(code_phase_estimate_navic_1bit)
            gps_code_phase_estimates_1bit.append(code_phase_estimate_gps_1bit)
            navic_doppler_estimates_1bit.append(doppler_estimate_navic_1bit)
            gps_doppler_estimates_1bit.append(doppler_estimate_gps_1bit)
            print("\n")

        navic_doppler_estimates_2bit = []
        gps_doppler_estimates_2bit = []
        navic_code_phase_estimates_2bit = []
        gps_code_phase_estimates_2bit = []
        for i in range(10):
            print(f"Satellite: {i + 1}")
            print(f"Actual Doppler: {doppler[i]}")
            print(f"Code Phase Delay: {timedelay[i]}")
            ...
            print("Navic Estimate 2 bit:")
            navic_estimates_2bit = code_phase_doppler_estimate(navic_sequence_2bit, 4e6, doppler_estimate_10, prn10_cpe_navic_complex,threshold)
            code_phase_estimate_navic_2bit = navic_estimates_2bit[0]
            doppler_estimate_navic_2bit = navic_estimates_2bit[1]

            print("GPS Estimate 2 bit:")
            gps_estimates_2bit = code_phase_doppler_estimate(gps_sequence_2bit, 4e6, doppler_estimate_10, prn10_cpe_gps_complex,threshold)
            code_phase_estimate_gps_2bit = gps_estimates_2bit[0]
            doppler_estimate_gps_2bit = gps_estimates_2bit[1]

            navic_code_phase_estimates_2bit.append(code_phase_estimate_navic_2bit)
            gps_code_phase_estimates_2bit.append(code_phase_estimate_gps_2bit)
            navic_doppler_estimates_2bit.append(doppler_estimate_navic_2bit)
            gps_doppler_estimates_2bit.append(doppler_estimate_gps_2bit)
            print("\n")
        ...
        for w in range(len(navic_code_phase_estimates_1bit)):
            average_code_phase_estimate_gps_1bit[w] += gps_code_phase_estimates_1bit[w] / num_iterations
            average_code_phase_estimate_navic_1bit[w] += navic_code_phase_estimates_1bit[w] / num_iterations
            average_doppler_estimate_gps_1bit[w] += gps_doppler_estimates_1bit[w] / num_iterations
            average_doppler_estimate_navic_1bit[w] += navic_doppler_estimates_1bit[w] / num_iterations

        for w in range(len(navic_code_phase_estimates_2bit)):
            average_code_phase_estimate_gps_2bit[w] += gps_code_phase_estimates_2bit[w] / num_iterations
            average_code_phase_estimate_navic_2bit[w] += navic_code_phase_estimates_2bit[w] / num_iterations
            average_doppler_estimate_gps_2bit[w] += gps_doppler_estimates_2bit[w] / num_iterations
            average_doppler_estimate_navic_2bit[w] += navic_doppler_estimates_2bit[w] / num_iterations
    write_estimates_to_file(
        "Repeatedexp/estimates",
        snr,
    ...
    write_estimates_to_file(
        "Repeatedexp/estimates_1bit_",
        snr,
        average_code_phase_estimate_gps_1bit,
        average_code_phase_estimate_navic_1bit,
        average_doppler_estimate_gps_1bit,
        average_doppler_estimate_navic_1bit
    )
    write_estimates_to_file(
        "Repeatedexp/estimates_2bit_",
        snr,
        average_code_phase_estimate_gps_2bit,
        average_code_phase_estimate_navic_2bit,
        average_doppler_estimate_gps_2bit,
        average_doppler_estimate_navic_2bit
    )