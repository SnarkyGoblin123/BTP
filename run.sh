...
FINAL_DEST_DIR="/home/joel/work/data"  # Replace with final destination folder path
...
for k in $(seq 0.01 0.01 0.01); do
    ...
    cd /home/joel/work/data
    sudo chmod 777 combined_data.dat
    cd ../

    # Run the GNSS-SDR tool with the specified config file and save the log output
    gnss-sdr --config_file=$CONFIG_FILE > log_1_alp.txt

    # Completion message
    echo "Process completed for k=$k. Combined data file moved to $FINAL_DEST_DIR/combined_data_k_${k}.dat. Log saved as log_${kalp}_k_${k}.txt"
